...
exports.getALLOrders = async (req, res, next) => {
    try {
        ...
        let results = await orderDb.find(findCri).populate({
            path: 'ticket_obj_id',
...
exports.assignTicketTechnician = async (req, res, next) => {
    try {
        ...
        let updatedDetails = await ticketDb.findOneAndUpdate(findCri, updateCri, { new: true })
        let ticketDetails = await ticketDb.findOne(findCri).populate('service_provided_for')



        let technicianMobileNumber = "91" + isTechnicianBelongToCenter.personal_details.phone.mobile_number
        let customerMobileNumber = "91" + isTicketFound.personal_details.primary_phone.mobile_number
        let ticketAddress = isTicketFound.address_details.address_line + " " + isTicketFound.address_details.pincode
        ...
        await notificationService.sendOtpViaMasssms(commonFolder.SMSDetails.TICKER_ASSIGNED, technicianMobileNumber, msg)
        await notificationService.sendOtpViaMasssms(commonFolder.SMSDetails.TICKET_INFO, technicianMobileNumber, ticketInfo)
...
exports.changeTicketTechnician = async (req, res, next) => {
    try {
        ...
        await notificationService.sendOtpViaMasssms(commonFolder.SMSDetails.TICKER_ASSIGNED, technicianMobileNumber, msg)
        await notificationService.sendOtpViaMasssms(commonFolder.SMSDetails.TICKER_ASSIGNED, customerMobileNumber, msg)
...
exports.closeTicket = async (req, res, next) => {
    try {
        ...
        let isTechnicianAssigned = await ticketDb.findOne(findCriDoesTechnicianAssigned)
        ...
        remarks?.close_ticket_remarks ? updateCri['remarks.close_ticket_remarks'] = remarks?.close_ticket_remarks : ''

        let updatedDetailsTicket = await ticketDb.findOneAndUpdate(findCriDoesTechnicianAssigned, updateCri, { new: true })
        ...
        await notificationService.sendOtpViaMasssms(commonFolder.SMSDetails.TICKER_CLOSURE, customerMobileNumber, msg)
...
exports.acceptBroadCastRequest = async (req, res, next) => {
    try {
        ...
        console.log(JSON.stringify(findCriteriaForMathcingTechnician) , "findCriteriaForMathcingTechnician")
...
exports.getAllTickets = async (req, res, next) => {
    try {
        ...
        let results = await ticketDb.find(crit).populate("service_provided_for").populate('assigned_ids.assigned_center_id')
...
exports.registerCenter = async (req, res, next) => {
    try {
        ...
        await notificationService.sendOtpViaMasssms(commonFolder.SMSDetails.CENTER_REGISTRATION, centerMobileNumber, msg)
...
exports.registerCenterViaApp = async (req, res, next) => {
    try {
        ...
        let { center_name, personal_details, primary_services, address_details, no_of_technicians,
            device_id } = req.body
        ...
        await centerServices.checkIfTechnicianOtpVerified(country_code, mobile_number)
...
exports.createTechnician = async (req, res, next) => {
    try {
        ...
        let createdTechnician = await centerServices.getTotalNoOfTechnicianInACenter(centerDetails._id)
...
exports.loginWithOtp = async (req, res, next) => {
    try {
        ...
        let centerDetails = await centerServices.checkIfCenterExists(country_code, mobile_number)
        ...
        let populatedCenterDetails = await centerDb.findOne({ _id: centerDetails._id }).populate('services.primary_services').populate('services.secondary_services.secondary_services_id').populate('clients_ids_list')
...
exports.verifyOtp = async (req, res, next) => {
    try {
        ...
        let verificationResult = await centerServices.checkIfTheOtpIsValid(country_code, mobile_number, otp)
...
exports.verifyOtpViaApp = async (req, res, next) => {
    try {
        ...
        let centerResult = await centerServices.checkIfTheOtpIsValidForCenterViaApp(country_code, mobile_number, otp);

        let technicianResult = await centerServices.checkIfTheOtpIsValidForTechnicianViaApp(country_code, mobile_number, otp);
        ...
        let centerDetails = await centerDb.findOneAndUpdate({ _id: centerResult._id }, updateCri, { new: true }).populate('services.primary_services').populate('services.secondary_services').populate('clients_ids_list')

        await technicianDb.findOneAndUpdate({ _id: technicianResult._id }, updateCri, { new: true });
        ...
        let populatedTechnicianDetails = await technicianDb.findOne({ _id: technicianResult._id }).populate('services.primary_services').populate('center_id')

        let token = commonFunctionForAuth.generateAccessToken(tokenEmbed)

        msg = "Otp has been verified successfully"
        let resData = { token, technicianResult, centerDetails, populatedTechnicianDetails }
...
exports.payPublicTicket = async (req, res , next) => {
    try{
        ...
        let updatedOrderStatus = await customerOrders.findOneAndUpdate(findCriteria, updateCriteria, { new: true })
       ...
       let updatedData = await ticketDb.findOneAndUpdate(criTicket, updateTicketCri, { new: true })

        if (updatedData?.created_by) {
            await centerServices.broadCastAllTicketsV1ForCreatedUsingQR(updatedData?.created_by, updatedData)
...
exports.PublicTicketBookingPaid = async (req, res, next) => {
    try {
        let { service_provided_for, specific_requirement, personal_details, address_details
            , time_preference, offers_applied, center_obj_id, authorized_client_id } = req.body
...
exports.updateTechnician = async (req, res, next) => {
    try {
        let { personal_details, primary_services, secondary_services, service_area_main_pincode,
            service_area_secondary_pincode, address_details_permanent,
            address_details_temporary, engagement_type, emergency_details,
            document_details, referenceDetails, center_obj_id, technician_obj_id
        } = req.body
